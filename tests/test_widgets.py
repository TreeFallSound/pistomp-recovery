...
from pistomp_recovery.ui.widgets.panel import Panel, TITLE_BAR_H
from pistomp_recovery.ui.widgets.text import TextWidget, ProgressBar, StatusLine
...
class TestMenuWidget:
    def test_menu_items(self, surface: pygame.Surface, fake_lcd, snapshot) -> None:
        menu: Menu = Menu(Box(4, 4, 312, 232), title="Recovery")
        menu.add_item("Resume", lambda _: None)
        menu.add_item("System Info", lambda _: None)
        menu.add_item("Package Updates", lambda _: None)
        menu.add_item("Config Management", lambda _: None)
        menu.add_item("Factory Reset", lambda _: None)
        menu.add_item("Reboot", lambda _: None)
        menu.add_item("Power Off", lambda _: None)
    ...
    def test_menu_scroll(
        self, surface: pygame.Surface, fake_lcd, snapshot
    ) -> None:
        menu: Menu = Menu(Box(4, 4, 312, 100), title="Select")
        for i in range(20):
            menu.add_item(f"Item {i + 1}", lambda _: None)
        for _ in range(10):
